...
float calculateError(const float data[], int dataLength,
    ...
    for (int i = 0; i < dataLength; i++) {
        float sineValue = amplitude * sin(2 * M_PI * frequency * (float) i * (1.0 / (1.4e6)) + phase) + offset;
...
void fitSineWave(const float data[], int dataLength, float frequency,
    ...
    // Quick estimate for the amplitude
    ...
    *bestOffset = O;
    ...
    for (float phase = phaseMin; phase <= phaseMax; phase += phaseStepLarge) {
		...
		float error = calculateError(data, dataLength, A, phase, O, frequency);
    ...
    for (float amplitude = amplitudeMin; amplitude <= amplitudeMax; amplitude += amplitudeStepLarge) {
		// Calculate the error for the current combination of parameters
		float error = calculateError(data, dataLength, amplitude, *bestPhase, *bestOffset, frequency);
